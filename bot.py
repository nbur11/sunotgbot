...
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, PreCheckoutQuery, LabeledPrice, FSInputFile, BufferedInputFile
...
# Имя владельца для договоров
OWNER_NAME = "Ваше ФИО"  # Замените на реальное ФИО
...
AGENT_IPS_FILE = f"{DATA_DIR}/agent_ips.json"
AGENT_CONFIRMATIONS_FILE = f"{DATA_DIR}/agent_confirmations.json"
...
class AdminPanel(StatesGroup):
    ...
    waiting_for_agent_approval = State()
...
def load_json(file_path):
    try:
        ...
        return {} if file_path in [USERS_FILE, ORDERS_FILE, AGENTS_FILE, REFERRAL_STATS_FILE, 
                                  AGENT_PAYMENTS_FILE, AGENT_IPS_FILE, AGENT_CONFIRMATIONS_FILE] else []
...
def get_user_ip_hash(message: types.Message):
    """Получает хеш IP адреса пользователя для проверки на дубликаты"""
    # В Telegram боте нет прямого доступа к IP, используем ID + время как альтернативу
    # В реальной системе нужно использовать webhook и получать IP из заголовков
    unique_string = f"{message.from_user.id}_{message.date}"
    return hashlib.sha256(unique_string.encode()).hexdigest()

def check_suspicious_agent(user_id, phone, card_number):
    """Проверяет агента на подозрительную активность"""
    agents = load_json(AGENTS_FILE)
    
    # Проверяем, не используется ли уже этот телефон или карта
    for agent_id, agent_data in agents.items():
        if agent_id != str(user_id):
            if agent_data.get("phone") == phone:
                return True, "Этот номер телефона уже используется другим агентом"
            if agent_data.get("card_number") == card_number:
                return True, "Эта карта уже используется другим агентом"
    
    return False, ""

def save_agent_ip(user_id, ip_hash):
    """Сохраняет IP хеш агента для проверки на мультиаккаунты"""
    ips = load_json(AGENT_IPS_FILE)
    if str(user_id) not in ips:
        ips[str(user_id)] = []
    
    if ip_hash not in ips[str(user_id)]:
        ips[str(user_id)].append(ip_hash)
    
    save_json(AGENT_IPS_FILE, ips)

# Функции для работы с агентами
def register_agent(user_id, username, first_name, agent_type, phone, card_number):
    agents = load_json(AGENTS_FILE)
    ...
    agents[str(user_id)] = {
        ...
        "verified": False,
        "agreement_accepted": datetime.now().isoformat()
    }
    ...
    if str(user_id) not in users:
        ...
        if referrer_code:
            ...
            if referrer_code in ref_stats:
                ...
                if agent_id and agent_data.get("status") == "active":
                    agents = load_json(AGENTS_FILE)
    ...
    if task_id in orders:
        ...
        if str(user_id) in users:
            ...
            if referrer_code:
                ...
                # Начисляем комиссию агенту только если он активен
                agent_id, agent_data = get_agent_by_ref_code(referrer_code)
                if agent_id and agent_data.get("status") == "active":
                    ...
                    # Сохраняем информацию о начислении
                    confirmations = load_json(AGENT_CONFIRMATIONS_FILE)
                    confirmation_id = str(uuid.uuid4())[:8]
                    confirmations[confirmation_id] = {
                        "agent_id": agent_id,
                        "user_id": user_id,
                        "task_id": task_id,
                        "order_amount": price_paid / 100,
                        "commission": commission,
                        "date": datetime.now().isoformat(),
                        "status": "pending"
                    }
                    save_json(AGENT_CONFIRMATIONS_FILE, confirmations)
                    ...
                    try:
                        asyncio.create_task(agent_bot.send_message(
                            ...
                            f"ID начисления: `{confirmation_id}`",
                            parse_mode="Markdown"
                        ))
                    except:
                        pass
...
def get_admin_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="👤 Агенты", callback_data="admin_agents")],
...
def get_agent_main_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💰 История выплат", callback_data="agent_payment_history")],
...
def generate_stats_chart():
    ...
    total_agent_earnings = sum(a.get("total_earned", 0) for a in agents.values())
...
Выплачено: {total_agent_earnings:.2f}₽"""
...
# Управление агентами (администратор)
@main_dp.callback_query(F.data == "admin_agents")
async def admin_agents_callback(callback: types.CallbackQuery):
    ...
    for agent_id, agent_data in agents.items():
        ...
        if status == "pending":
            ...
            blocked_agents.append(agent_info)
    
    # Показываем ожидающих одобрения
    if pending_agents:
        text += "⏳ **Ожидают одобрения:**\n"
        for agent in pending_agents[:5]:
            agent_data = agent["data"]
            text += f"• {agent_data['first_name']} ({agent_data['type']}) - ID: `{agent['id']}`\n"
        if len(pending_agents) > 5:
            text += f"... и еще {len(pending_agents) - 5}\n"
        text += "\n"
    ...
    if active_agents:
        ...
        for agent in active_agents[:10]:
            agent_data = agent["data"]
            text += f"• {agent_data['first_name']} - Баланс: {agent_data['balance']:.2f}₽\n"
        if len(active_agents) > 10:
            text += f"... и еще {len(active_agents) - 10}\n"
        text += "\n"
    
    # Показываем заблокированных
    if blocked_agents:
        text += "🚫 **Заблокированные:**\n"
        for agent in blocked_agents[:5]:
            agent_data = agent["data"]
            text += f"• {agent_data['first_name']} - ID: `{agent['id']}`\n"
        if len(blocked_agents) > 5:
            text += f"... и еще {len(blocked_agents) - 5}\n"
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Одобрить агентов", callback_data="admin_approve_agents")],
        [InlineKeyboardButton(text="🚫 Заблокировать агента", callback_data="admin_block_agent")],
...
@main_dp.callback_query(F.data == "admin_approve_agents")
async def admin_approve_agents(callback: types.CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await safe_callback_answer(callback)
        return
    ...
    await safe_edit_message(
        callback.message,
        "✅ **Одобрение агентов**\n\n"
        "Введите ID агента для одобрения:",
        parse_mode="Markdown"
    )
    await state.set_state(AdminPanel.waiting_for_agent_approval)
    await safe_callback_answer(callback)

@main_dp.message(AdminPanel.waiting_for_agent_approval)
async def process_agent_approval(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    ...
    if agent_data.get("status") == "active":
        await message.answer("ℹ️ Агент уже активен")
    else:
        ...
        try:
            await agent_bot.send_message(
                ...
                "Теперь вы можете начать привлекать клиентов и зарабатывать комиссию.\n\n"
                "Используйте личный кабинет для отслеживания статистики.",
                parse_mode="Markdown",
        ...
        await message.answer(
            f"✅ Агент {agent_data['first_name']} одобрен и активирован!",
            reply_markup=get_admin_keyboard()
        )
    ...
    if agent_id in agents:
        ...
        if agent_data.get("status") == "pending":
            await message.answer(
                ...
                "Администратор проверит ваши данные и активирует аккаунт в ближайшее время.\n\n"
                "Вы получите уведомление после одобрения.",
                parse_mode="Markdown"
            )
        elif agent_data.get("status") == "blocked":
            await message.answer(
...
async def process_agent_type(callback: types.CallbackQuery, state: FSMContext):
    ...
    await safe_edit_message(
        ...
        f"Введите ваше полное имя (ФИО или название компании):",
        parse_mode="Markdown"
    )
...
async def process_agent_card(message: types.Message, state: FSMContext):
    ...
    is_suspicious, reason = check_suspicious_agent(message.from_user.id, phone, card)
    if is_suspicious:
        await message.answer(f"❌ {reason}")
        await state.clear()
        return
    
    # Показываем договор для подтверждения
    agent_type_names = {
        'individual': 'Физическое лицо',
        'ip': 'ИП',
        'selfemployed': 'Самозанятый'
    ...
    agreement_text = f"""📄 **ДОГОВОР-ОФЕРТА**

**Исполнитель:** {OWNER_NAME} (самозанятый)
**Агент:** {name}
...
2. Выплаты производятся на указанную карту после подтверждения
3. Минимальная сумма для вывода: 100₽
4. Агент обязуется не использовать недобросовестные методы привлечения клиентов

**ВАЖНО:**
Я, {name}, подтверждаю, что:
• Получаю вознаграждение за привлечение клиентов/размещение рекламы
• Обязуюсь самостоятельно декларировать доход и уплачивать налоги в соответствии с законодательством РФ
• {OWNER_NAME} выплачивает вознаграждение на указанные реквизиты после подтверждения заказа

Нажимая "Принять договор", вы соглашаетесь с условиями."""
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Принять договор", callback_data="accept_agreement")],
        [InlineKeyboardButton(text="❌ Отклонить", callback_data="reject_agreement")]
    ])
    
    await message.answer(agreement_text, parse_mode="Markdown", reply_markup=keyboard)
    
    await state.update_data(card=card)
    await state.set_state(AgentRegistration.waiting_for_agreement)

@agent_dp.callback_query(F.data == "accept_agreement")
async def accept_agreement(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    
    if not data:
        await callback.answer("❌ Ошибка. Начните регистрацию заново.", show_alert=True)
        await state.clear()
        return
    ...
    # Сохраняем IP хеш
    ip_hash = get_user_ip_hash(callback.message)
    save_agent_ip(callback.from_user.id, ip_hash)
    ...
    await agent_bot.send_photo(
        chat_id=callback.message.chat.id,
        photo=BufferedInputFile(qr_code.getvalue(), filename="qr_code.png"),
        caption=f"✅ **Регистрация завершена!**\n\n"
                f"⏳ Ваша заявка отправлена на модерацию.\n"
                f"Администратор проверит ваши данные в течение 24 часов.\n\n"
                f"После одобрения вы сможете:\n"
                f"• Привлекать клиентов по вашей ссылке\n"
                f"• Получать 30% с каждой оплаченной песни\n"
                f"• Выводить средства на указанную карту\n\n"
                f"📄 Договор сохранен в системе.",
        parse_mode="Markdown"
    )
    
    # Уведомляем админов о новом агенте
    admin_text = f"""🆕 **Новая заявка агента**
...
Для одобрения используйте админ-панель."""
    
    for admin_id in [OWNER_ID, ADMIN_ID]:
        try:
            await main_bot.send_message(admin_id, admin_text, parse_mode="Markdown")
        except:
            pass
...
@agent_dp.callback_query(F.data == "reject_agreement")
async def reject_agreement(callback: types.CallbackQuery, state: FSMContext):
    await safe_edit_message(
        callback.message,
        "❌ Вы отклонили договор.\n\n"
        "Для участия в агентской программе необходимо принять условия договора.",
        parse_mode="Markdown"
    )
    await state.clear()
    await safe_callback_answer(callback)
...
async def agent_cabinet(callback: types.CallbackQuery):
    ...
    if agent_data.get("status") != "active":
        await callback.answer("⏳ Ваш аккаунт еще не активирован", show_alert=True)
        return
...
async def agent_stats(callback: types.CallbackQuery):
    ...
    if agents[agent_id].get("status") != "active":
        await callback.answer("⏳ Ваш аккаунт еще не активирован", show_alert=True)
        return
...
async def agent_withdraw(callback: types.CallbackQuery, state: FSMContext):
    ...
    await safe_edit_message(
        ...
        f"Карта для вывода: {agent_data['card_number'][:4]} **** **** {agent_data['card_number'][-4:]}\n\n"
...
ID заявки: `{payment_id}`"""
        ...
        admin_keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Подтвердить выплату", callback_data=f"confirm_payment_{payment_id}")],
        ...
        await message.answer(
            ...
            f"После перевода средств администратором вам придет уведомление о необходимости подтвердить получение.",
            parse_mode="Markdown",
            reply_markup=get_agent_main_keyboard()
        )
...
@agent_dp.callback_query(F.data == "agent_payment_history")
async def agent_payment_history(callback: types.CallbackQuery):
    agent_id = str(callback.from_user.id)
    agents = load_json(AGENTS_FILE)
    ...
    payments = load_json(AGENT_PAYMENTS_FILE)
    agent_payments = [p for p in payments.values() if p["agent_id"] == agent_id]
    ...
    # Сортируем по дате
    agent_payments.sort(key=lambda x: x["created_date"], reverse=True)
    
    text = "📋 **История выплат:**\n\n"
    
    for payment in agent_payments[:10]:
        status_emoji = {
            "pending": "⏳",
            "confirmed": "💸",
            "completed": "✅",
        ...
        date = payment["created_date"][:10]
        text += f"{status_emoji} {date} - {payment['amount']:.2f}₽\n"
        
        if payment["status"] == "pending":
            text += "   Ожидает обработки\n"
        elif payment["status"] == "confirmed":
            text += "   Ожидает подтверждения получения\n"
    ...
    if len(agent_payments) > 10:
        text += f"... и еще {len(agent_payments) - 10} выплат"
...
3. Минимальная сумма для вывода: 100₽
4. Агент обязуется не использовать спам и недобросовестную рекламу
...
- Самостоятельно декларировать доход и платить налоги

**ОБЯЗАННОСТИ ИСПОЛНИТЕЛЯ:**
- Своевременно начислять вознаграждение
- Производить выплаты по заявкам агента
- Предоставлять актуальную статистику

**ОТВЕТСТВЕННОСТЬ:**
За нарушение условий договора агент может быть заблокирован."""
...
1. Пройдите регистрацию и дождитесь одобрения
2. Получите вашу уникальную ссылку и QR-код
3. Делитесь ими с потенциальными клиентами
4. Получайте 30% с каждой оплаченной песни

**Где взять материалы:**
• Ссылка и QR-код - в личном кабинете
• Можно сохранить QR-код и использовать в рекламе
• Ссылку можно размещать в соцсетях

**Как происходят выплаты:**
1. Накопите минимум 100₽ на балансе
2. Создайте заявку на вывод
3. Администратор переведет деньги
4. Подтвердите получение

**Важные правила:**
• Не используйте спам для привлечения
• Не обещайте то, чего нет в сервисе
• Платите налоги с полученного дохода
• Одна ссылка - один агент (нельзя создавать подагентов)

**Советы по продвижению:**
• Покажите примеры созданных песен
• Расскажите о возможности создать уникальный подарок
• Подчеркните простоту использования
• Используйте праздники и события как повод

По всем вопросам обращайтесь в поддержку основного бота."""
    
    await safe_edit_message(callback.message, help_text, parse_mode="Markdown", reply_markup=get_agent_main_keyboard())
    await safe_callback_answer(callback)

# Подтверждение получения выплаты агентом
@agent_dp.callback_query(F.data.startswith("confirm_receipt_"))
async def confirm_payment_receipt(callback: types.CallbackQuery):
    payment_id = callback.data.replace("confirm_receipt_", "")
    payments = load_json(AGENT_PAYMENTS_FILE)
    ...
    if payment["agent_id"] != str(callback.from_user.id):
        await callback.answer("❌ Это не ваш платеж", show_alert=True)
        return
    
    if payment["status"] != "confirmed":
        await callback.answer("❌ Платеж не в статусе ожидания подтверждения", show_alert=True)
        return
    
    # Обновляем статус
    payments[payment_id]["status"] = "completed"
    payments[payment_id]["completed_date"] = datetime.now().isoformat()
    save_json(AGENT_PAYMENTS_FILE, payments)
    
    # Обновляем договор-подтверждение
    confirmations = load_json(AGENT_CONFIRMATIONS_FILE)
    confirmation_id = str(uuid.uuid4())[:8]
    ...
    confirmations[confirmation_id] = {
        ...
        "date": datetime.now().isoformat(),
        "text": f"Я, {callback.from_user.first_name}, подтверждаю получение {payment['amount']:.2f}₽ за привлечение клиентов. Обязуюсь самостоятельно уплатить налоги."
    }
    save_json(AGENT_CONFIRMATIONS_FILE, confirmations)
    ...
    for admin_id in [OWNER_ID, ADMIN_ID]:
        try:
            await main_bot.send_message(
                admin_id,
                f"✅ Агент подтвердил получение {payment['amount']:.2f}₽\n"
...
@main_dp.callback_query(F.data.startswith("confirm_payment_"))
async def confirm_agent_payment(callback: types.CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Нет доступа", show_alert=True)
        return
    
    payment_id = callback.data.replace("confirm_payment_", "")
    
    await safe_edit_message(
        callback.message,
        f"💸 **Подтверждение выплаты**\n\n"
        f"ID платежа: `{payment_id}`\n\n"
        f"Переведите средства на карту агента и введите 'да' для подтверждения:",
        parse_mode="Markdown"
    )
    
    await state.update_data(payment_id=payment_id)
    await state.set_state(AdminPanel.waiting_for_payment_confirmation)
    await safe_callback_answer(callback)

@main_dp.message(AdminPanel.waiting_for_payment_confirmation)
async def process_payment_confirmation(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    
    if message.text.lower() != "да":
        await message.answer("❌ Подтверждение отменено", reply_markup=get_admin_keyboard())
        await state.clear()
        return
    ...
    if payment_id not in payments:
        await message.answer("❌ Платеж не найден", reply_markup=get_admin_keyboard())
        await state.clear()
        return
    ...
    payments[payment_id]["confirmed_date"] = datetime.now().isoformat()
    payments[payment_id]["confirmed_by"] = message.from_user.id
    save_json(AGENT_PAYMENTS_FILE, payments)
    ...
    # Уведомляем агента с кнопкой подтверждения
    try:
        await agent_bot.send_message(
            int(agent_id),
            f"💸 **Средства отправлены!**\n\n"
            f"Сумма: {amount:.2f}₽\n"
            f"Карта: {payment['card']}\n\n"
            f"После получения средств нажмите кнопку ниже для подтверждения:",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="✅ Подтвердить получение", callback_data=f"confirm_receipt_{payment_id}")]
            ])
        )
    except:
        pass
    ...
    await message.answer(
        f"✅ Выплата подтверждена и отправлена агенту!\n"
        f"Ожидаем подтверждения получения от агента.",
        reply_markup=get_admin_keyboard()
    )
